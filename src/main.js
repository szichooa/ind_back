...
router.beforeEach((to,from,next) => {   
	if(to.matched.some( m => m.meta.auth)){        
		if(window.localStorage.isLogin=='1'&&window.localStorage.isAdminId=="1") { // 已经登陆       
		next()   // 正常跳转到你设置好的页面     
		}else if(window.sessionStorage.isLogin=='1'&&window.localStorage.isAdminId=="1") { // 已经登陆       
		next()   // 正常跳转到你设置好的页面     
		}else if(window.sessionStorage.isLogin=='1'||window.localStorage.isLogin=='1'&&window.localStorage.isAdminId=="2"){
		next()
		}else{
		next({path:'/login'})
