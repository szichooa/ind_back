<template>
	<div class="content-views">
		<div class="content-views-nav">
			<p class="title-view">计算数据库</p>
			<div class="mbanav-r">
			<el-breadcrumb separator-class="el-icon-arrow-right">
				<el-breadcrumb-item :to="{ path: '/index' }">首页</el-breadcrumb-item>
				<el-breadcrumb-item>计算数据库</el-breadcrumb-item>
			</el-breadcrumb>
			</div>
		</div>
		...
		<div class="content-views-body">
			<el-table ref="multipleTable"
				...
				<el-table-column prop="funName" label="绑定函数名字" :show-overflow-tooltip="true">
				</el-table-column>
		...
		<div class="content-view-alert" v-if="alertIsShow">
			...
			<el-form :model="addOpeForm" ref="addOpeForm" label-width="100px" class="add-addOpeForm-form demo-addOpeForm" size="small" v-loading="loadingA">
				...
				<el-form-item label="绑定函数" v-if="addOpeForm.btnIsAdd">
					<el-select v-model="addOpeForm.funID" placeholder="请选择" @change="selectChange" style="width:100%">
						<el-option
...
import {ossBaseURL_g,ossDownBaseURL_g,addDataListDB,delDataListDB,getDataListDB,editDataListDB,getAllFncList } from '../../api/api';
	...
	export default {
		methods: {
			showDataBase(row){
				...
				ichooarouter.push('/data/datashow/'+rows.id);
			},
			//绑定数据
			bindDataBase(row){
				let rows=row;
				window.sessionStorage.setItem('dataBaseName',rows.dbName)
				window.sessionStorage.setItem('funBindID',rows.funId)
				ichooarouter.push('/data/bind/'+rows.id);
			},
			//添加业务
			addDataBaseAlert(formName){
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						params.append('fun_id',this.addOpeForm.funID);
						addDataListDB(window.sessionStorage.ApiUrl,params).then((res)=>{
			...
			editDataBaseAlert(formName){
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						editDataListDB(window.sessionStorage.ApiUrl,params).then((res)=>{
							if(res.data.status==200){						
								this.$message({
								type: 'success',
								message: '添加成功!'
			...
			deleteRow(row) {
				...
				this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
					...
					delDataListDB(window.sessionStorage.ApiUrl,{
