...
<template>
	<div class="content-views">
		...
		<div class="content-views-body">
			...
			<div class="content-views-body-cont">
				<el-form class="input-data" :model="dataRelationList" ref="dataRelationList" size="small">
					<h4>关系绑定</h4>
					<div class="input-data-l">
						<el-form-item v-for="itemI,indexI in dataRelationList.inputData"
						:key="itemI.id"
						:prop="'inputData.'+indexI+'.attrName'"
						:rules="{required: true, message: '参数值不能为空', trigger: 'blur'}">
							<el-input v-model="itemI.attrName" placeholder="数据名称" :inline="true" class="data-input-value" style="width:180px;height:32px;"></el-input>
						</el-form-item>
					</div>
					<div class="input-data-r">
						<el-form-item v-for="itemI,indexI in dataRelationList.inputData"
						:key="itemI.id"
						style="position:relative;"
						:prop="'inputData.'+indexI+'.reportAttr'"
						:rules="{required: true, message: '参数值不能为空', trigger: 'blur'}">
							<el-input v-model="itemI.reportAttr" placeholder="数据位置" :inline="true" class="data-input-value" style="width:180px;height:32px;"></el-input>
							<el-button type="text" @click="delItemI(indexI)" class="delete-btn" icon="el-icon-delete" style="position:absolute;right:-5px;"></el-button>
						</el-form-item>
					</div>
...
import {addDataBind,getDataBind } from '../../../api/api';
	export default{
		data() {
			return {
				dataRelationList:{
		...
		mounted(){
			...
			getDataBind(window.sessionStorage.ApiUrl,{
		...
		methods:{
			delItemI(indexI){
				if(this.dataRelationList.inputData.length>1){
			...
			editDataRelation(){
				this.$refs['dataRelationList'].validate((valid) => {
					if (valid) {
						var params = new URLSearchParams();
						params.append('fun_id', window.sessionStorage.funBindID);
						...
						addDataBind(window.sessionStorage.ApiUrl,params).then(res=>{
							if(res.data.status==200){
								this.$message({
									type: 'success',
									message: '保存成功!'
